...
describe '[STEP2] ユーザログイン後のテスト' do
  ...
  describe 'ヘッダーのテスト: ログインしている場合' do
    context 'リンクの内容を確認: ※logoutは『ユーザログアウトのテスト』でテスト済みになります。' do
      ...
      it 'Homeを押すと、自分のユーザ詳細画面に遷移する' do
        ...
        expect(subject).to eq "/users/#{user.id}"
      end
  ...
  describe '投稿一覧画面のテスト' do
    ...
    context '投稿成功のテスト' do
      ...
      it 'リダイレクト先が、保存できた投稿の詳細画面になっている' do
        ...
        expect(page).to have_current_path "/books/#{Book.last.id}", ignore_query: true
      end
    end
  end
  ...
  describe '自分の投稿詳細画面のテスト' do
    ...
    context '編集リンクのテスト' do
      it '編集画面に遷移する' do
        click_link 'Edit'
        expect(page).to have_current_path "/books/#{book.id}/edit", ignore_query: true
      end
    end
