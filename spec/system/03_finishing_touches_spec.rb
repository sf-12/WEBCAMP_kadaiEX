...
describe '[STEP3] 仕上げのテスト' do
  ...
  describe 'サクセスメッセージのテスト' do
    ...
    it 'ユーザ新規登録成功時' do
      ...
      expect(subject).to have_content 'successfully'
    end
  ...
  describe '処理失敗時のテスト' do
    ...
    context '投稿データの更新失敗: titleを空にする' do
      ...
      it '投稿編集画面を表示しており、フォームの内容が正しい' do
        expect(page).to have_current_path "/books/#{book.id}", ignore_query: true
        expect(find_field('book[title]').text).to be_blank
  ...
  describe '他人の画面のテスト' do
    ...
    describe '他人の投稿詳細画面のテスト' do
      ...
      context '表示内容の確認' do
        it 'URLが正しい' do
          expect(page).to have_current_path "/books/#{other_book.id}", ignore_query: true
        end
    ...
    describe '他人のユーザ詳細画面のテスト' do
      ...
      context '表示の確認' do
        it 'URLが正しい' do
          expect(page).to have_current_path "/users/#{other_user.id}", ignore_query: true
        end
  ...
  describe 'グリッドシステムのテスト: container, row, col-xs-〇を正しく使えている' do
    ...
    it 'ユーザ一覧画面' do
      visit users_path
      expect(subject).to have_selector '.container .row .col-md-3'
      expect(subject).to have_selector '.container .row .col-md-8.offset-md-1'
    end
  ...
  describe 'アイコンのテスト' do
    context 'トップ画面' do
      ...
      it '本のアイコンが表示される' do
        expect(subject).to have_selector '.fas.fa-book'
      end
    end
    ...
    context 'ヘッダー: ログインしていない場合' do
      ...
      it 'Homeリンクのアイコンが表示される' do
        expect(subject).to have_selector '.fas.fa-home'
      end

      it 'Aboutリンクのアイコンが表示される' do
        expect(subject).to have_selector '.fas.fa-link'
      end

      it 'sign upリンクのアイコンが表示される' do
        expect(subject).to have_selector '.fas.fa-user-plus'
      end

      it 'loginリンクのアイコンが表示される' do
        expect(subject).to have_selector '.fas.fa-sign-in-alt'
      end
    end
    ...
    context 'ヘッダー: ログインしている場合' do
      ...
      it 'Usersリンクのアイコンが表示される' do
        expect(subject).to have_selector '.fas.fa-users'
      end

      it 'Booksリンクのアイコンが表示される' do
        expect(subject).to have_selector '.fas.fa-book-open'
      end

      it 'log outリンクのアイコンが表示される' do
        expect(subject).to have_selector '.fas.fa-sign-out-alt'
      end
    end
    ...
    context 'サイドバー' do
      ...
      it 'ユーザ一覧画面でレンチアイコンが表示される' do
        visit users_path
        expect(subject).to have_selector '.fas.fa-user-cog'
      end
